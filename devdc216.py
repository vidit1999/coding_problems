...
# dict keys are towns 
# and value is another dict with keys 'mean' and 'variacne'
...
def to_dict_load(data):
    alltowns = data.split("\n")
    townwise = {}
    for i in range(len(alltowns)):
        town_name, values = alltowns[i].split(":")
        values = values.split(",")
        mon_vals = {}
        ...
        mon_vals['mean'] = mean
        mon_vals['variance'] = variance
        townwise[town_name] = mon_vals
    return townwise
...
def test_all(data):
    for t in towns:
        print(f"{t} : Mean -> {mean(t,data)}, Variance -> {variance(t, data)}")
